...
| `INSTANCE_ID` | Identifiant unique de votre instance (généré au premier lancement ou manuellement) | *(vide)* |

<Callout type="info" title="Fonctionnement">
La vérification de licence est principalement **offline** (signature Ed25519 embarquée dans la clé). Un refresh en ligne est effectué toutes les 24h auprès du serveur de licences. En cas d'indisponibilité du serveur, une **période de grâce de 7 jours** maintient les features EE actives.
</Callout>
