...
Provisionnez les addons PostgreSQL et Redis, puis renseignez les variables d'environnement (au minimum `SECURITY_JWT_SECRET`, `SECURITY_WEBHOOK_SECRET`, `DATABASE_URL=$SCALINGO_POSTGRESQL_URL`, `REDIS_URL=$SCALINGO_REDIS_URL`, `NEXT_PUBLIC_SITE_URL`, `AUTH_TRUST_HOST=1`).
