...
Pour un déploiement orchestré sur instance [Coolify](https://coolify.io) self-hostée (Docker Compose, Traefik, ressources managées Postgres/Redis), la procédure recommandée est décrite dans [Déploiement automatisé](/doc/technical/deployment#coolify-self-host). Architecture supportée : wildcard DNS via Traefik (provider `gandiv5` pour Gandi LiveDNS), MinIO comme stockage S3-compatible, review apps éphémères par PR avec DB dédiée et préfixe S3 par PR (`STORAGE_S3_KEY_PREFIX=pr-<n>/`).

### Docker

L'application ne fournit pas de Dockerfile officiel public, mais le standalone output se prête bien à la conteneurisation :
