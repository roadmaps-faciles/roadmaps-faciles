...
Les variables préfixées `NEXT_PUBLIC_` sont injectées au build et exposées au navigateur ; les autres sont lues au runtime côté serveur.
...
L'adresse `from` est résolue dans cet ordre : `MAILER_FROM_EMAIL` s'il est défini, sinon le login SMTP (`MAILER_SMTP_LOGIN`), sinon `noreply@<domaine de l'instance>` (dérivé de `NEXT_PUBLIC_SITE_URL`).
...
| Variable | Description | Défaut |
|----------|-------------|--------|
| `NEXT_PUBLIC_BRAND_NAME` | Nom du service | `Roadmaps Faciles` |
| `NEXT_PUBLIC_BRAND_TAGLINE` | Accroche sous le nom | *(défaut RMF)* |
| `NEXT_PUBLIC_BRAND_MINISTRY` | Bloc-marque de l'État (le `\n` force le retour à la ligne, ex : `Ministère\nde l'Exemple`) | `République\nFrançaise` |
| `NEXT_PUBLIC_BRAND_OPERATOR_ENABLE` | Afficher le logo opérateur (`true`/`false`) | `true` |
| `NEXT_PUBLIC_BRAND_OPERATOR_LOGO_URL` | URL du logo opérateur | `/img/roadmaps-faciles.png` |
| `NEXT_PUBLIC_BRAND_OPERATOR_LOGO_ALT` | Texte alternatif du logo | `Roadmaps Faciles` |
| `NEXT_PUBLIC_BRAND_OPERATOR_LOGO_ORIENTATION` | Orientation du logo (`vertical`/`horizontal`) | `vertical` |
...
| Variable | Description | Défaut |
|----------|-------------|--------|
| `NEXT_PUBLIC_LEGAL_PUBLISHER_NAME` | Éditeur du site | `Roadmaps Faciles` |
...
| `NEXT_PUBLIC_LEGAL_HOSTING_CONTACT` | Contact de l'hébergeur | `support@scalingo.com` |
| `NEXT_PUBLIC_LEGAL_HOSTING_PRIVACY_URL` | Politique de confidentialité de l'hébergeur | *(Scalingo)* |
| `NEXT_PUBLIC_LEGAL_CONTACT_EMAIL` | Email de contact public | `contact@roadmaps-faciles.fr` |
| `NEXT_PUBLIC_LEGAL_RGPD_EMAIL` | Email du délégué à la protection des données | `rgpd@roadmaps-faciles.fr` |
...
- **RGPD / CNIL** : renseignez toutes les variables `NEXT_PUBLIC_LEGAL_*`, désignez un DPO joignable via `NEXT_PUBLIC_LEGAL_RGPD_EMAIL`, et tenez à jour la politique de confidentialité (`/politique-de-confidentialite`).
...
- **Tracking CNIL-compatible** : si vous activez l'analytique, choisissez une solution conforme (Matomo auto-hébergé sans cookie), ou désactivez le tracking avec `NEXT_PUBLIC_TRACKING_PROVIDER=noop`. Évitez les solutions transférant des données hors UE sans encadrement.
