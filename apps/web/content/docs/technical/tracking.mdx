...
Le tracking repose sur une **abstraction multi-provider** dans `src/lib/tracking-provider/`. Trois fournisseurs sont supportés, sélectionnés via la variable d'environnement `TRACKING_PROVIDER` :
...
En développement local, le provider par défaut est `noop` - aucun événement n'est envoyé. Pour tester le tracking, configurez `TRACKING_PROVIDER=posthog` avec une clé PostHog valide.
</Callout>
...
|----------|-------------|---------|
| `TRACKING_PROVIDER` | Provider actif | `noop`, `matomo`, `posthog` |
| `POSTHOG_KEY` | Clé projet PostHog | `phc_...` |
| `POSTHOG_HOST` | Host PostHog (EU) | `https://eu.i.posthog.com` |
| `MATOMO_URL` | URL serveur Matomo | `https://stats.example.fr` |
| `MATOMO_SITE_ID` | ID site Matomo | `42` |
