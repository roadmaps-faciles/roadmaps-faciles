...
export const Tour = ({ userFullName }: Props) => {
  ...
  const steps: JoyrideProps["steps"] = [
    ...
    },
    {
      ...
      title: "Site settings",
      content:
        'Click "Site settings" to customize your feedback space. You can add custom boards and statuses, configure various settings, personalize appearance, and more.',
      disableBeacon: true,
    },
    {
    ...
    },
    {
      ...
      title: "That's all!",
      content:
        "We hope Astuto will help you understand your customers and make better decisions! You can always replay this tour from here.",
      disableBeacon: true,
    },
  ];
  ...
  return (
    <Joyride
      ...
      callback={state => {
        ...
        if (
          vw < BOOTSTRAP_BREAKPOINT_SM &&
          state.type === "step:after" && //(state.action === 'next' && state.step.target === '.boardsNav') || // This causes positioniting problems for Joyride tour
          state.action === "prev" &&
          state.step.target === ".boardsNav"
        ) {
          closeBoardsNav();
        }
        ...
        if (
          state.type === "step:after" &&
          (((state.action === "next" || state.action === "close") &&
            (state.step.target === ".sidebarFilters" ||
              state.step.target === ".siteSettingsDropdown" ||
              state.step.target === ".moderationDropdown")) ||
            (state.action === "prev" &&
              (state.step.target === ".moderationDropdown" || state.step.target === ".tourDropdown")))
        ) {
