...
- Canonical redirect: `PLATFORM_DOMAIN` env var (e.g. `scalingo.io`) - proxy 301-redirects platform default domain to `SITE_URL`; skips `/api/` routes and the case where rootDomain IS itself on the platform domain (avoids redirect loops); empty = disabled
...
- Prisma 7 `prisma migrate dev` does NOT auto-load `.env` - it reads `datasource.url` from `prisma.config.ts` which itself reads `process.env.DATABASE_URL`. Clean fix: load the env inside `prisma.config.ts` so the CLI sees it however it's invoked - `apps/web` uses `@next/env` (`loadEnvConfig(__dirname, true)`), `apps/licensing` uses a dev-gated `await import("dotenv")` + `config()`. Gate both on `NODE_ENV === "development" || !NODE_ENV` so prod (env injected by the platform) skips the load and `dotenv` stays a devDependency. Avoid the `node --env-file=... node_modules/.bin/prisma migrate dev` hack: under pnpm `.bin/prisma` is a POSIX shell shim (not a JS symlink), so `node node_modules/.bin/prisma` crashes parsing `basedir=$(dirname ...)` as JS.
...
- SVG sizing dans les `Button` shadcn (et `Tabs`, `SegmentedControl`, `Dialog/Sheet` close…) : l'utility `ui-svg-icon` (`packages/ui/src/tokens/utilities.css`) force `size-4` (16px) sur tout `<svg>` enfant via `[&_svg:not([class*='size-'])]:size-4`. Conséquence : le prop `size={N}` (lucide-react, `@icons-pack/react-simple-icons`…) pose `width`/`height` en **attributs HTML** sur le `<svg>`, écrasés par le CSS du parent. **Toujours utiliser `className="size-N"`** au lieu du prop `size` quand le SVG est rendu dans un de ces composants : le selector `:not([class*='size-'])` se désactive dès qu'une classe `size-*` est présente. Hors de ces composants, `size={N}` fonctionne normalement.
- Tailwind 4 dark mode : par défaut Tailwind 4 utilise `@media (prefers-color-scheme: dark)` (suit l'OS), pas la classe `.dark` qu'on toggle via `ThemeScript`. Pour aligner Tailwind sur la stratégie class du projet, déclarer `@custom-variant dark (&:where(.dark, .dark *));` juste après `@import "tailwindcss"` dans `apps/web/src/app/tailwind-entry.css`. Sans ça, les utilities `dark:*` se déclenchent via la media query OS, indépendamment du toggle de l'app : bug latent qui se manifeste quand l'OS est en dark et l'app en light (ou inverse). Modifier `@custom-variant` nécessite `rm -rf apps/web/.next/cache` + restart `pnpm dev` (Turbopack cache la résolution Tailwind).
...
- Config publique RUNTIME : les vars publiques non-`NEXT_PUBLIC_` (`SITE_URL`, `BRAND_*`, `LEGAL_*`, `REPOSITORY_URL`, `MATOMO_*`, `TRACKING_PROVIDER`, `POSTHOG_*`) sont lues au runtime de façon isomorphique via `isoEnv()` dans `src/config.ts` (navigateur → `window.__PUBLIC_CONFIG__`, serveur → `process.env[key]` en **accès dynamique** non inliné). `PublicConfigScript` (`next/script` `beforeInteractive`, comme `ThemeScript`) injecte le global AVANT les bundles. Permet à l'image prébuildée de servir n'importe quel domaine/branding sans rebuild. **Toute nouvelle var publique lue côté client doit être ajoutée à `PUBLIC_ENV_KEYS`** sinon `isoEnv` renvoie `undefined` dans le navigateur. Restent build-time (inlinés) : `NEXT_PUBLIC_APP_*` (artefacts d'image), `NEXT_PUBLIC_SENTRY_DSN` (Sentry client). Rappel Next : un `NEXT_PUBLIC_*` est gelé au build (même côté serveur), l'env runtime est ignoré
- `SITE_URL` malformé (ex: `https://` quand le domaine est vide) crashe le serveur au module-eval : `new URL(logo, config.host)` dans `shared-metadata.ts` + `metadataBase` dans `layout.tsx` lèvent `Invalid URL`. Toujours fournir un `SITE_URL` avec un host complet
- Coolify compose : `${PUBLIC_DOMAIN}` dans le bloc `environment:` est interpolé au **parse-time** depuis le `.env` généré par Coolify, où les vars magiques (`COOLIFY_FQDN`/`COOLIFY_URL`) et les vars de l'UI n'atterrissent PAS de façon fiable → mettre les valeurs (`SITE_URL`, etc.) en **littéral** directement dans le compose, pas via `${PUBLIC_DOMAIN}`/`${COOLIFY_FQDN}`
...
- Corepack pin : pas besoin de `corepack prepare pnpm@X.Y.Z --activate` dans les Dockerfiles. `corepack enable` seul suffit - au premier appel `pnpm` dans un stage qui a copié `package.json`, corepack lit `packageManager` et télécharge la version. Plus de drift à maintenir si on bump pnpm
- pnpm node_modules + COPY entre stages : les `node_modules` pnpm contiennent des **symlinks vers `.pnpm/`** qui se cassent au COPY entre stages Docker (le COPY suit les liens et duplique). Pour packager une app non-standalone (ex: `apps/licensing/` Hono), utiliser `pnpm deploy --prod /target` qui produit un dossier autosuffisant sans symlinks
- Prisma CLI au runtime : ne pas utiliser `npx --yes prisma@7` dans un entrypoint container - télécharge le binaire à chaque cold start (~10s + dépendance npm registry). Bake le CLI dans l'image runner en copiant `node_modules/.bin/prisma` + `node_modules/prisma` + `node_modules/@prisma` depuis le builder stage
- Wildcard cert Let's Encrypt via Gandi : Traefik supporte le provider `gandiv5` nativement (DNS-01 challenge). Token Gandi avec scope `Manage DNS records`, env var `GANDIV5_PERSONAL_ACCESS_TOKEN` côté proxy. Couvre `*.domaine.fr` en un seul cert (limite Let's Encrypt : 50 certs/semaine par registered domain)
