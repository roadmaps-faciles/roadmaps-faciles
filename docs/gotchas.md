...
- Canonical redirect: `PLATFORM_DOMAIN` env var (e.g. `scalingo.io`) - proxy 301-redirects platform default domain to `NEXT_PUBLIC_SITE_URL`; skips `/api/` routes and the case where rootDomain IS itself on the platform domain (avoids redirect loops); empty = disabled
