...
Toutes les variables ci-dessous sont lues dans [`apps/web/src/config.ts`](../../apps/web/src/config.ts) **au runtime** : branding, pages légales, tracking, URL du site. Aucune n'est inlinée au build, donc l'image GHCR prébuildée sert n'importe quel domaine et n'importe quel branding sans rebuild ; vous les posez en variables d'environnement runtime sur votre instance. Les valeurs publiques (branding, légal, tracking) sont exposées au navigateur via une config runtime injectée par le serveur.
...
3. Sinon, `noreply@<domaine de l'instance>` (dérivé de `SITE_URL`).
...
| Variable | Défaut | Rôle |
|----------|--------|------|
| `BRAND_NAME` | `Roadmaps Faciles` | Nom du service |
| `BRAND_TAGLINE` | `Du feedback mutuel...` | Accroche |
| `BRAND_MINISTRY` | `République\nFrançaise` | Bloc marque de l'État (le `\n` force le retour à la ligne, ex : `Ministère\nde l'Exemple`) |
| `BRAND_OPERATOR_ENABLE` | `true` | Affiche le logo opérateur |
| `BRAND_OPERATOR_LOGO_URL` | `/img/roadmaps-faciles.png` | URL du logo opérateur |
| `BRAND_OPERATOR_LOGO_ALT` | `Roadmaps Faciles` | Texte alternatif du logo |
| `BRAND_OPERATOR_LOGO_ORIENTATION` | `vertical` | `horizontal` ou `vertical` |
...
| Variable | Défaut | Rôle |
|----------|--------|------|
| `LEGAL_PUBLISHER_NAME` | `Roadmaps Faciles` | Éditeur du site |
| `LEGAL_PUBLISHER_ADDRESS` | (vide) | Adresse de l'éditeur |
| `LEGAL_PUBLICATION_DIRECTOR` | `Le responsable légal...` | Directeur de la publication |
| `LEGAL_HOSTING_NAME` | `Scalingo SAS` | Nom de l'hébergeur |
| `LEGAL_HOSTING_ADDRESS` | `15 avenue du Rhin...` | Adresse de l'hébergeur |
| `LEGAL_HOSTING_CONTACT` | `support@scalingo.com` | Contact de l'hébergeur |
| `LEGAL_HOSTING_PRIVACY_URL` | (Scalingo) | URL de la politique de l'hébergeur |
| `LEGAL_CONTACT_EMAIL` | `contact@roadmaps-faciles.fr` | Email de contact public |
| `LEGAL_RGPD_EMAIL` | `rgpd@roadmaps-faciles.fr` | Email du délégué à la protection des données |
...
- [ ] **RGPD / CNIL** : renseigner toutes les variables `LEGAL_*`, désigner un DPO joignable via `LEGAL_RGPD_EMAIL`, et tenir à jour la politique de confidentialité (`/politique-de-confidentialite`).
...
- [ ] **Tracking CNIL-compatible** : si vous activez l'analytique, choisir une solution conforme (Matomo auto-hébergé sans cookie, ou désactiver le tracking avec `TRACKING_PROVIDER=noop`). Éviter les solutions transférant des données hors UE sans encadrement.
...
| Branding | `BRAND_MINISTRY`, `BRAND_OPERATOR_*` |
| Légal | `LEGAL_*` |
| Stockage | `STORAGE_PROVIDER`, `STORAGE_S3_*` |
| Conformité | `TRACKING_PROVIDER` |
