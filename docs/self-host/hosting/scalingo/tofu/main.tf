...
resource "scalingo_app" "licensing" {
  name = "${var.project_name}-licensing-${var.environment}"
  ...
  environment = {
    ...
    ADMIN_API_KEY                 = var.licensing_admin_api_key
  }
}

# --- Addon licensing ---

resource "scalingo_addon" "licensing_postgresql" {
  app_id      = scalingo_app.licensing.id
  provider_id = "postgresql"
  plan        = var.licensing_db_plan
}

# --- Domaine licensing ---

resource "scalingo_domain" "licensing" {
  count  = var.domain != "" ? 1 : 0
  app_id = scalingo_app.licensing.id
  name   = "licensing.${var.domain}"
}

# --- Container sizing licensing ---

resource "scalingo_container_type" "licensing" {
  app_id = scalingo_app.licensing.id
  name   = "web"
  size   = var.licensing_container_size
  amount = 1
}
