...
- **Licensing self-host** : vérification offline Ed25519 + refresh 24h + grace 7j. Détection cloud vs self-host via `!await getEffectiveLicenseKey()` (override cookie possible en dev). Côté cloud, UI admin `/admin/licensing` (tabs `Statut` / `Gestion` en mode cloud) avec table + filtres + create/detail/revoke/renew dialogs, et UI verify de clé arbitraire (cloud-only : le bouton "Vérifier" est masqué en self-host, et l'appel `/verify` du test passe `dryRun=true` pour ne pas enregistrer l'instanceId ni déclencher un faux warning multi-instance). La clé privée du serveur licensing (`LICENSING_ED25519_PRIVATE_KEY`) DOIT correspondre à la clé publique embarquée `publicKey.ts` (paire `generate-keypair.ts`), sinon signature invalide offline alors que l'online passe.
- **Thème DSFR : deux verrous orthogonaux** : (1) orga = entitlement `THEME_DSFR` (`GOV_LICENSED` en self-host, plan `GOV` en cloud), (2) tenant = `customDomain` en `.gouv.fr` (charte DSFR). Les deux requis. Vérifs : `assertEntitlement` dans l'action + `SaveTenantWithSettings` (rejet si pas `.gouv.fr`) + cohérence inverse dans `UpdateTenantDomain` (interdit de retirer le `.gouv.fr` tant que DSFR actif) + UI `GeneralForm` (option désactivée + message). Le thème par défaut reste `Default`.
- **Bootstrap self-host** : `POST /api/setup` (gardé par `SETUP_TOKEN`, idempotent) crée org + tenant + admin OWNER via `createMinimalInstance` (`src/lib/bootstrap.ts`), car l'image standalone ne peut pas lancer `prisma db seed`. Aussi dispo en dev via `SEED_MINIMAL=1 prisma db seed`. Côté licensing server (apps/licensing) : routes `/admin/*` Bearer-protégées pour list/detail/create/revoke/renew/verifications. Dev tools `/admin/dev-tools` (dev only) pour issue+bind sur instance courante via cookie override (`DEV_LICENSE_KEY_COOKIE`, `DEV_LICENSE_OFFLINE_COOKIE`), force expire et verify clé. Flows d'émission : LICENSED via Stripe Checkout, GOV_LICENSED via admin manuel (qualification gouv). Audit actions : `ROOT_LICENSE_ADMIN_CREATE/REVOKE/RENEW`. Code : `src/lib/ee/licensing/`, `src/app/admin/(ee)/licensing/`, `src/app/admin/dev-tools/`. Détail : [docs/architecture/licensing.md](docs/architecture/licensing.md)
...
- CI: GitHub Actions (`.github/workflows/` : build, lint, test, docker-build)
  - Build/deploy: `.github/workflows/docker-build.yml` builds the web image **multi-arch (amd64 + arm64)** and pushes it to GHCR. Build runs per-arch on native runners (`ubuntu-latest` + `ubuntu-24.04-arm`, no QEMU) pushing by digest, then a `merge` job assembles the multi-arch manifest with tags. Tags: push `main` → `main` (mutable, no per-commit sha, staging image of the trunk); tag `v*` → `vX.Y.Z` + semver (`X.Y.Z`, `X.Y` on stable) + `latest`. Coolify pulls the image at runtime (manual); the SaaS cutover from Scalingo to Coolify is done (`deploy.yml` + `.github/scripts/resolve-deploy.js` removed). On tag `v*` a `gate` job runs `.github/scripts/ci-gate-release.js` (poll Build/Lint/Tests green on the tagged commit before building); skipped on push `main` and dispatch (staging not gated). The licensing image stays built by Coolify (source-decrypt git-crypt stage, BSL code not pushed to a registry)
  ...
  - Safety net: on `push` to main, all jobs always run regardless of path filters
...
- Deployment (Coolify) : la SaaS tourne sur Coolify (cutover Scalingo terminé ; `deploy.yml`/`.slugignore` retirés). `Dockerfile` multi-stage à la racine (Prisma CLI bundlé), `scripts/docker-entrypoint.sh` run migrate deploy, standalone build path `apps/web/.next/standalone/apps/web/server.js`, wildcard DNS Gandi via Traefik gandiv5 resolver. Déploiement Scalingo self-host réactivé via `Procfile` + `scalingo.json` racine (buildpack ; bootstrap d'instance via `POST /api/setup`, pas de seed au déploiement). Config SaaS-spécifique chiffrée dans `docs/deploy/coolify/` (git-crypt) ; templates self-host génériques publics dans `docs/self-host/` (docker-compose + Garage, Coolify, Scalingo, Scaleway tofu, Caddy)
